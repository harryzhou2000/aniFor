...
The composed-output gate samples representative energy, dense solid, liquid, and gas regions. Keep dense fluid-core coverage, bounded luma range, and clipping checks in both Canvas2D and WebGL. The homogeneous lower Water/Oil/Acid/Lava columns additionally enforce bounded adjacent-pixel micro-contrast and stable family hue so raw-cell glitter or clipped Lava cannot return; semantic unit tests alone cannot prove that a shader, compositor, or fallback draw order still presents the field.
