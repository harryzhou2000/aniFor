...
The next substantial work is framework leverage, not another isolated material
detail. Build a small declarative experiment layer that centralizes selector
name, query/default policy, parent capability, output-scale budget, uniform or
packed-selector binding, and canvas dataset diagnostics. Migrate a narrow
accepted slice first and require identical emitted state and pixels before
expanding it.

In parallel, replace cloned one-off visual audit orchestration with a reusable
visual-lab comparison route: deterministic fixture preparation, off/on capture,
named regions, screenshot paths, and a compact report should be data, while
semantic/topology assertions remain optional adapters. This is for fast human
A/B decisions; subjective colour and lighting work does not need a new broad
unit-test matrix before checkpointing.

Keep shader source modular at authoring time even if Pixi still receives one
assembled GLSL string. Extract coherent normal-fragment sections behind a
deterministic assembler and source-map labels, without changing order or
generated bytes in the first checkpoint. Track each module's samplers,
neighbour reads, persistent resources, and supported Detail rungs so compact
8× exclusions are explicit rather than rediscovered through compiler failure.
Once these seams exist, use short experiment branches or consecutive checkpoint
commits for competing powder/liquid/gas treatments and select winners from
