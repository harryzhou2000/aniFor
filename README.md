...
The toolbox's **Detail** row switches between true 1×, 2×, 4×, and 8× per-cell output (the same values accepted by `?renderScale=`). The 8× path draws the semantic shader directly on one quad, uses only a 2× Canvas during cold WebGL startup, and releases all fallback contour storage after promotion. It is intended for high-quality captures; 2× remains the default and 4× the practical routine high-detail mode.
