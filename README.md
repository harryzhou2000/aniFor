...
Realistic WebGL also applies one shared scale-safe body finish to already-proven powder, liquid, and gas volumes. The normal 1×–4× compositor and compact true-8× mesh inject the same GLSL helper, so their broad upper-left key, opposing fill, dense-core absorption, and pigment retention share one vocabulary even though HDR and bloom remain normal-scale only. At normal detail the Off/Balanced/Volumetric profile scales those already-computed generic key, fill, and pigment terms once across all three phases; it replaces former phase-specific profile deltas instead of compounding another lighting layer. Compact true 8× passes literal Off. Phase-specific fields still own density, contacts, alpha, topology, and material identity; the finish itself is RGB-only and adds no render target or pass. Reconstructed gas reuses its existing propagated identity style, while reconstructed liquid uses one fixed native-world nearest-filtered R8 optical-class plane so connected empty support can select the authored family directly without becoming a semantic particle, performing another palette lookup, or allocating an 8× resource. Classic disables the finish through the existing product-volume switch.

Normal and true-8× WebGL remain the canonical material presentation. Canvas resolves the same authored phase/class profile only to tune transport already present in its liquid/gas volume helper: fill scales absorption and transmission scales scatter. Key and pigment response, RGB lifts, rims, and topology remain native to each backend. This RGB-only adapter adds no allocation, texture, field, pass, alpha, support, or ownership change.

The same shared source now adds a mesoscopic fluid-volume lobe for liquids and gases. Each compositor reuses its already-live centre density, cardinal mean, field-native curvature, depth, and slope: convex fluid shoulders receive a broad cool crown, while concave or deep pockets retain coloured absorption. The ordinary normal-HDR Realistic profile scales those existing terms through its Off/Balanced/Volumetric material-lighting value, giving connected liquids clearer exposed transmission and core depth while adding restrained mid-volume key and extinction to gas. Volumetric adds a soft, squared shallow-zone transmission shelf after reflection scaling, so Water and Oil retain their authored transmission response and gain visible surface-to-core separation without a hard stripe. It also strengthens only gas's already-proven crown/shoulder key and pocket/core absorption, making cloud centres fuller without another noise field or material-specific shader branch. Compact true 8× keeps the profile at literal Off so its bounded compositor ABI and cost remain unchanged. Semantic particle density may prove visible support but cannot steer the curvature, so reconstructed gas fringes and liquid shores follow the authoritative atmosphere/liquid fields. The lobe changes RGB only and adds no sampler, uniform, field, upload, target, pass, material-ID branch, or 8×-scaled allocation; Classic disables it with the same existing volume switch.
