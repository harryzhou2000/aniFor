...
const normalizeProfile = (profile, driverName, version) => {
  ...
  if (hasCompletion) {
    ...
    const completionBind = version === 'v4' || version === 'v5'
      || version === 'v6' || version === 'v7' || version === 'v8' || version === 'v9'
  ...
  if (hasReadinessActivation) {
    assertExactDataKeys(
      readinessActivation,
      version === 'v7' || version === 'v9' ? V7_READINESS_ACTIVATION_FIELDS
        : version === 'v6' ? V6_READINESS_ACTIVATION_FIELDS : READINESS_ACTIVATION_FIELDS,
      `${label}.readinessActivation`,
    );
    const expectedCapability = version === 'v7' || version === 'v9'
    ...
    const expectedCompletionScope = version === 'v7' || version === 'v9'
...
/** Creates the closed v9 render-field readiness plus selection alpha-readback registry. */
export function createVisualCaptureExecutionV9CapabilityRegistry(drivers, profiles) {
  return createCapabilityRegistry(drivers, profiles, 'v9');
}
...
/** Returns the frozen v9 render-field readiness plus selection alpha-readback subset. */
export function visualCaptureExecutionV9CapabilitiesForCaptureOrder(driverNames) {
  return capabilitiesForCaptureOrder(driverNames, V9_REGISTRY);
}
...
const FIXTURE_ACTIVATION_RENDER_FIELD_AND_ALPHA_READBACK_PROFILE = () => {
  const activation = FIXTURE_ACTIVATION_RENDER_FIELD_GENERATION_PROFILE();
  const readback = SELECTION_OWNED_RECEIPT_AND_ALPHA_READBACK_PROFILE();
  return {
    ...
    framebufferReadback: readback.framebufferReadback,
...
const V9_PROFILE_REGISTRY = Object.fromEntries(
  DECLARED_DRIVER_NAMES.map((name) => [name, FIXTURE_ACTIVATION_RENDER_FIELD_AND_ALPHA_READBACK_PROFILE()]),
);
...
const V9_REGISTRY = createVisualCaptureExecutionV9CapabilityRegistry(
  VISUAL_CAPTURE_STATIC_CONTRACT.drivers,
  V9_PROFILE_REGISTRY,
);
...
export const VISUAL_CAPTURE_EXECUTION_V9_CAPABILITY_NAMES = V9_REGISTRY.names;
export const VISUAL_CAPTURE_EXECUTION_V9_CAPABILITIES = V9_REGISTRY.capabilities;
...
export function resolveVisualCaptureExecutionV9Capabilities(name) {
  return V9_REGISTRY.resolve(name);
}
