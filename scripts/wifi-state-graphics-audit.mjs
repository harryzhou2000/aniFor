const CHANNELS = [0, 3, 25, 100];
const EXPECTED_CARDS = CHANNELS.flatMap((channel) => [
  { key: `channel${channel}Off`, channel, active: false },
]).concat(CHANNELS.map((channel) => ({ key: `channel${channel}Active`, channel, active: true })));
const WIFI_MATERIAL = 152;
const WATER_MATERIAL = 2;
const PRESENT = 0x8000;
const ACTIVE = 0x0080;

/** Canonical WebGL proof for exact native WIFI channels and activity state. */
export async function auditWifiStateGraphics({
  cdp, mode, evaluate, waitFor, waitForStablePageCapture, captureSettledPage, outputScale = 2, assert,
}) {
  if (mode !== 'webgl') return { skippedCanvasFallback: true };
  const settle = outputScale === 8
    ? (label) => captureSettledPage(cdp, label, 450)
    : (label) => waitForStablePageCapture(cdp, label);
  await settle('WebGL initial WIFI-state framebuffer');
  await evaluate(cdp, `(() => {
    const audit = window.__ANIFOR_INPUT_AUDIT__;
    if (typeof audit.prepareWifiStateGraphicsFixture !== 'function'
      || typeof audit.wifiStateGraphicsAtlas !== 'function'
      || typeof audit.setWifiStateStyling !== 'function') {
      throw new Error('WIFI state graphics audit API unavailable');
    }
    audit.resetView();
    audit.prepareWifiStateGraphicsFixture();
    return true;
  })()`);
  const atlas = await waitFor(() => evaluate(cdp, `(() => {
    const cards = window.__ANIFOR_INPUT_AUDIT__.wifiStateGraphicsAtlas()?.cards;
    return cards?.length === ${EXPECTED_CARDS.length} ? cards : false;
  })()`), 15_000, 'WebGL WIFI state fixture');
  assert(atlas.every((entry, index) => matchesCard(entry, EXPECTED_CARDS[index])),
    `WIFI state atlas contract drifted (${JSON.stringify(atlas)})`);

  const semantic = await evaluate(cdp, `(() => {
    const audit = window.__ANIFOR_INPUT_AUDIT__;
    return audit.wifiStateGraphicsAtlas().cards.map((entry) => {
      const at = (rect) => ({
        material: audit.cell(rect.x, rect.y), state: audit.presentationState(rect.x, rect.y),
        wall: audit.wall(rect.x, rect.y),
      });
      return {
        body: at(entry.body), hole: at(entry.authoredHole), notch: at(entry.openNotch),
        thin: at(entry.thinStructure), isolated: at(entry.isolated), absent: at(entry.absentState),
        wrong: at(entry.wrongOwner), wall: at(entry.wallCoexistence),
      };
    });
  })()`);
  assert(semantic.every((entry, index) => {
    const expected = EXPECTED_CARDS[index];
    const state = PRESENT | expected.channel | (expected.active ? ACTIVE : 0);
    return entry.body.material === WIFI_MATERIAL && entry.body.state === state
      && entry.hole.material === 0 && entry.hole.state === 0
      && entry.notch.material === 0 && entry.notch.state === 0
      && entry.thin.material === WIFI_MATERIAL && entry.thin.state === state
      && entry.isolated.material === WIFI_MATERIAL && entry.isolated.state === state
      && entry.absent.material === WIFI_MATERIAL && entry.absent.state === 0
      && entry.wrong.material === WATER_MATERIAL && entry.wrong.state === state
      && entry.wall.material === WIFI_MATERIAL && entry.wall.state === state && entry.wall.wall === 1;
  }), `WIFI fixture lost owner/state/topology/wall separation (${JSON.stringify(semantic)})`);

  const setStyling = (enabled) => evaluate(cdp,
    `window.__ANIFOR_INPUT_AUDIT__.setWifiStateStyling(${enabled}); true;`);
  await setStyling(false);
  await settle('flat WIFI-state framebuffer');
  const flat = await sample(cdp, evaluate);
  await setStyling(true);
  await settle('styled WIFI-state framebuffer');
  const styled = await sample(cdp, evaluate);
  await setStyling(false);
  await settle('repeated flat WIFI-state framebuffer');
  const repeated = await sample(cdp, evaluate);

  const cards = atlas.map((entry, index) => {
    const response = subtract(styled.probes[index], flat.probes[index]);
    const repeatedResponse = subtract(repeated.probes[index], flat.probes[index]);
    assert(equal(flat.probes[index], repeated.probes[index]),
      `WIFI/${entry.key}: off→on→off is not exact`);
    assert(equal(flat.absent[index], styled.absent[index]) && equal(flat.absent[index], repeated.absent[index]),
      `WIFI/${entry.key}: absent owner marker changed`);
    assert(equal(flat.wrong[index], styled.wrong[index]) && equal(flat.wrong[index], repeated.wrong[index]),
      `WIFI/${entry.key}: state styling leaked into wrong owner`);
    assert(equal(flat.walls[index], styled.walls[index]) && equal(flat.walls[index], repeated.walls[index]),
      `WIFI/${entry.key}: state styling leaked through native wall`);
    assert(flat.probes[index][3] === styled.probes[index][3],
      `WIFI/${entry.key}: state style changed semantic alpha`);
    return {
      key: entry.key,
      channel: entry.channel,
      active: entry.active,
      peak: peak(response),
      repeatPeak: peak(repeatedResponse),
      styledRgb: styled.probes[index].slice(0, 3),
    };
  });
  for (const card of cards) {
    assert(card.peak >= 6 && card.peak <= 176 && card.repeatPeak === 0,
      `WIFI/${card.key} state is inert, unbounded, or unstable (${JSON.stringify(cards)})`);
  }
  for (const channel of CHANNELS) {
    const off = cards.find((card) => card.channel === channel && !card.active);
    const active = cards.find((card) => card.channel === channel && card.active);
    assert(off && active && peak(subtract(off.styledRgb, active.styledRgb)) >= 8,
      `WIFI channel ${channel} lost its active/off distinction (${JSON.stringify(cards)})`);
  }
  assert(peak(subtract(cards[0].styledRgb, cards[3].styledRgb)) >= 8,
    `WIFI channels 0 and 100 collapsed to one appearance (${JSON.stringify(cards)})`);
  return { cards, exactControls: true, exactRepeatedOff: true, rgbOnly: true };
}

async function sample(cdp, evaluate) {
  return evaluate(cdp, `(() => {
    const audit = window.__ANIFOR_INPUT_AUDIT__;
    const canvas = document.querySelector('canvas.semantic-field-canvas');
    const gl = canvas?.getContext('webgl2') || canvas?.getContext('webgl');
    if (!canvas || !gl) throw new Error('WIFI state WebGL backing unavailable');
    const rect = canvas.getBoundingClientRect();
    const read = (point) => {
      const screen = audit.worldToScreen(point.x + point.width / 2, point.y + point.height / 2);
      const x = Math.max(0, Math.min(canvas.width - 1,
        Math.floor((screen.x - rect.left) * canvas.width / rect.width)));
      const y = Math.max(0, Math.min(canvas.height - 1,
        canvas.height - 1 - Math.floor((screen.y - rect.top) * canvas.height / rect.height)));
      const pixel = new Uint8Array(4);
      gl.readPixels(x, y, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, pixel);
      return Array.from(pixel);
    };
    const cards = audit.wifiStateGraphicsAtlas().cards;
    return {
      probes: cards.map((entry) => read(entry.responseProbe)),
      absent: cards.map((entry) => read(entry.absentState)),
      wrong: cards.map((entry) => read(entry.wrongOwner)),
      walls: cards.map((entry) => read(entry.wallCoexistence)),
    };
  })()`);
}

function matchesCard(entry, expected) {
  return entry.material === WIFI_MATERIAL && entry.code === 'WIFI'
    && entry.key === expected.key && entry.channel === expected.channel && entry.active === expected.active
    && entry.encodedState === (PRESENT | expected.channel | (expected.active ? ACTIVE : 0));
}
function subtract(left, right) { return left.map((value, index) => value - right[index]); }
function peak(values) { return Math.max(...values.map((value) => Math.abs(value))); }
function equal(left, right) { return left.length === right.length && left.every((value, index) => value === right[index]); }
