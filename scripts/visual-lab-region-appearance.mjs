...
const sampleRegionAppearance = (image, rect) => {
  let sum = 0;
  let squaredSum = 0;
  let minimum = 255;
  let maximum = 0;
  ...
  return {
    ...
    luma: {
      ...
      verticalPairs,
      mean,
      standardDeviation: Math.sqrt(Math.max(0, squaredSum / pixels - mean * mean)),
...
const regionEvidence = (descriptor, images, candidate) => {
  ...
  const variants = Object.fromEntries(VARIANTS.map((variant) => [
    variant, sampleRegionAppearance(images[variant], rects[variant]),
  ]));
  const pair = (left, right) => ({
    left,
    right,
    signedLumaSumDelta: variants[right].luma.sum - variants[left].luma.sum,
    signedLumaMeanDelta: variants[right].luma.mean - variants[left].luma.mean,
    signedLumaStandardDeviationDelta: (
      variants[right].luma.standardDeviation - variants[left].luma.standardDeviation
    ),
    signedLumaNeighbourAbsoluteMeanDelta: (
      variants[right].luma.neighbourAbsoluteMean
        - variants[left].luma.neighbourAbsoluteMean
    ),
  });
  return {
    ...descriptor,
...
/**
 * Builds optional current-only spatial appearance evidence from a complete
 * batch. It describes local luminance distribution and neighbouring contrast;
 * it assigns no visual score, threshold, verdict, ranking, or promotion rule.
 */
export async function createVisualLabCurrentRegionAppearance(batch, readCapture) {
  if (batch?.schema !== BATCH_SCHEMA || batch.complete !== true
    ...
    throw new TypeError(`region appearance requires a complete ${BATCH_SCHEMA}`);
  }
  if (typeof readCapture !== 'function') {
    throw new TypeError('region appearance requires a capture reader');
  }
  ...
  for (const entry of batch.candidates) {
    ...
    if (result?.schema !== RESULT_SCHEMA || !SHA256_ID.test(result.id ?? '') || seen.has(result.id)) {
      throw new TypeError(`region appearance requires a unique result for ${entry.candidate}`);
    }
    seen.add(result.id);
    ...
    for (const variant of VARIANTS) {
      images[variant] = pinAndDecode(
        ...
        `${entry.candidate} ${variant} region appearance`,
      );
    }
    candidates.push({
