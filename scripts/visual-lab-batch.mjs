...
export function renderVisualLabRegionAppearanceBoard(appearance) {
  ...
  const cards = appearance.candidates.map((candidate) => {
    const regions = candidate.regions.map((region) => {
      ...
      const crops = VARIANTS.map((variant) => {
        ...
        return `<figure><a href="${escapeHtml(source)}"><span class="crop" style="width:${rect.width}px;height:${rect.height}px"><img src="${escapeHtml(source)}" loading="lazy" alt="${escapeHtml(`${candidate.candidate} ${region.name} ${variant}`)}" style="left:-${rect.x}px;top:-${rect.y}px"></span></a><figcaption>${escapeHtml(variant.toUpperCase())}</figcaption></figure>`;
      }).join('');
  ...
  return `<!doctype html>\n<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Visual Lab region appearance</title><style>:root{color-scheme:dark;font-family:system-ui,sans-serif;background:#11151b;color:#eaf0f7}body{margin:0 auto;max-width:1800px;padding:24px}article{background:#1b222c;border:1px solid #344252;border-radius:12px;padding:16px;margin:18px 0}.regions{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,420px),1fr));gap:14px}.region{min-width:0;background:#121820;border:1px solid #2d3a48;border-radius:9px;padding:12px}.region header{display:flex;justify-content:space-between;gap:10px;align-items:baseline}.region h3,.region p{margin:0}.region p{color:#aebdcd;font-size:.82rem;text-align:right}.crops{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px;margin-top:10px;overflow:auto}.crops figure{margin:0;min-width:0}.crop{display:block;position:relative;overflow:hidden;max-width:100%;background:#080b0f;border:1px solid #344252}.crop img{display:block;position:absolute;max-width:none;width:auto;height:auto}figcaption{text-align:center;color:#b8c5d2;font-size:.78rem;margin-top:3px}dl{display:flex;flex-wrap:wrap;gap:6px 14px;margin:10px 0 0}dl div{display:flex;gap:5px}dt{color:#9eb0c2}dd{margin:0;font-variant-numeric:tabular-nums}@media(max-width:700px){body{padding:12px}.region header{display:block}.region p{text-align:left;margin-top:3px}}</style></head><body><h1>Current region appearance</h1><p>OFF/A/B crops and local luminance measurements navigate this batch only. They expose texture, edges, and tonal variation without scoring or deciding aesthetics, and never pin visuals across revisions. Select a crop to open its authenticated full capture. Full data is in <a href="./region-appearance.json">region-appearance.json</a>.</p>${cards}</body></html>\n`;
}
