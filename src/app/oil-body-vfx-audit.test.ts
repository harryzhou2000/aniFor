...
import {
  ...
  type OilBodyVfxBoundary,
  type OilBodyVfxPoint,
  type OilBodyVfxRect,
  type OilBodyVfxWallPattern,
} from './oil-body-vfx-audit';

const WORLD_WIDTH = 612;
const WORLD_HEIGHT = 384;
const world: OilBodyVfxRect = { x: 0, y: 0, width: WORLD_WIDTH, height: WORLD_HEIGHT };

describe('oil body VFX audit fixture', () => {
  it('pins paired broad exact-Oil panes and exact vertical optical-depth bands', () => {
    expect(OIL_BODY_VFX_AUDIT.panes.map(({ code, material, backing }) => (
      [code, material, backing.kind]
    ))).toEqual([
      ['OPEN_POOL', Material.Oil, 'air'],
      ['WALL_CONTROL', Material.Oil, 'native-wall-checker'],
    ]);
    ...
    for (const pane of OIL_BODY_VFX_AUDIT.panes) {
    ...
    const wallPane = OIL_BODY_VFX_AUDIT.panes.find(
    ...
    if (wallPane?.backing.kind === 'native-wall-checker') {
      // A clear four-row run below an occupied wall block can reach only 18;
      // neither half of the checker is evidence of a deep Oil body.
      expect(depthRange(depth, wallPane.backing.region)).toEqual({ min: 0, max: 18 });
    }
  });

  it('direct-fills exact pane topology, sparse Oil, material controls, seams, and contacts', () => {
    const simulation = preparedFixture();
    const cells = simulation.cells();

    for (const pane of OIL_BODY_VFX_AUDIT.panes) {
      expectRectExcept(cells, pane.body, Material.Oil, [
        pane.authoredHole,
        pane.openChimney,
      ...
      expectFourCardinals(cells, pane.reconstructablePinhole, Material.Oil);
    }

    const { sparse, materialControls, seams, contacts } = OIL_BODY_VFX_AUDIT;
    expectRect(cells, sparse.thinStrand, Material.Oil);
    expectRect(cells, sparse.droplet, Material.Oil);
    expect(materialAt(cells, sparse.isolated)).toBe(Material.Oil);
    ...
    expectRect(cells, OIL_BODY_VFX_AUDIT.guardedBlank, Material.Empty);

    expect(materialCount(cells, Material.Oil)).toBe(98_479);
    expect(materialCount(cells, Material.Diesel)).toBe(5_888);
    expect(materialCount(cells, Material.Nitro)).toBe(4_608);
    expect(materialCount(cells, Material.Water)).toBe(5_888);
    for (const material of [Material.Acid, Material.Lava, Material.Soap]) {
  ...
  it('authors one aligned 3,072-cell checker without changing exact Oil ownership', () => {
    const simulation = preparedFixture();
    ...
    expect(wallAt(walls, wallPane.backing.wallProbe)).toBe(OIL_BODY_VFX_AUDIT.conductiveWall);
  ...
  it('keeps every independent region disjoint and inside the 612x384 world', () => {
    const fixture = OIL_BODY_VFX_AUDIT;
    const independent: OilBodyVfxRect[] = [
      ...
      ...Object.values(fixture.seams).flatMap(({ oil, other }) => [oil, other]),
      ...Object.values(fixture.contacts).flatMap(({ oil, other }) => [oil, other]),
  ...
  it('resets cells and walls deterministically and rejects unsupported backends', () => {
    ...
    prepareOilBodyVfxFixture(first);
    expect(firstDifference(first.cells(), expectedCells)).toBe(-1);
    expect(firstDifference(first.walls(), expectedWalls)).toBe(-1);

    expect(() => prepareOilBodyVfxFixture(new RenderLabBackend(32, 32))).toThrow('requires 612x384');
    expect(() => prepareOilBodyVfxFixture(new DeterministicBackend(WORLD_WIDTH, WORLD_HEIGHT)))
...
function preparedFixture(): RenderLabBackend {
  ...
  prepareOilBodyVfxFixture(simulation);
  return simulation;
}

function materialAt(cells: Uint8Array, point: OilBodyVfxPoint): Material {
  return cells[point.y * WORLD_WIDTH + point.x] as Material;
}

function wallAt(walls: Uint8Array, point: OilBodyVfxPoint): number {
  return walls[point.y * WORLD_WIDTH + point.x];
}

function depthAt(depth: Uint8Array, point: OilBodyVfxPoint): number {
  return depth[point.y * WORLD_WIDTH + point.x];
}

function depthRange(depth: Uint8Array, rect: OilBodyVfxRect): { min: number; max: number } {
  let min = 255;
  let max = 0;
...
function expectBoundary(cells: Uint8Array, entry: OilBodyVfxBoundary): void {
  expectRect(cells, entry.oil, Material.Oil);
  expectRect(cells, entry.other, entry.otherMaterial);
  expect(entry.oil.x + entry.oil.width).toBe(entry.other.x);
  expect(materialAt(cells, entry.oilProbe)).toBe(Material.Oil);
  expect(materialAt(cells, entry.otherProbe)).toBe(entry.otherMaterial);
  expect(entry.oilProbe.x + 1).toBe(entry.otherProbe.x);
}

function expectRect(cells: Uint8Array, rect: OilBodyVfxRect, material: Material): void {
  let mismatch: OilBodyVfxPoint | undefined;
  forEachPoint(rect, (point) => {
...
function expectRectExcept(
  ...
  rect: OilBodyVfxRect,
  material: Material,
  exceptions: readonly OilBodyVfxRect[],
): void {
  let mismatch: OilBodyVfxPoint | undefined;
  forEachPoint(rect, (point) => {
...
function expectFourCardinals(cells: Uint8Array, point: OilBodyVfxPoint, material: Material): void {
...
function expectWallPattern(
  ...
  let materialMismatch: OilBodyVfxPoint | undefined;
  let wallMismatch: OilBodyVfxPoint | undefined;
  forEachPoint(pattern.region, (point) => {
    if (!materialMismatch && materialAt(cells, point) !== Material.Oil) materialMismatch = point;
    ...
    const expected = (blockX + blockY) % 2 === pattern.occupiedParity
      ? OIL_BODY_VFX_AUDIT.conductiveWall : 0;
...
function rectInside(inner: OilBodyVfxRect, outer: OilBodyVfxRect): boolean {
  return inner.x >= outer.x && inner.y >= outer.y
    && inner.x + inner.width <= outer.x + outer.width
    && inner.y + inner.height <= outer.y + outer.height;
}

function pointInside(point: OilBodyVfxPoint, outer: OilBodyVfxRect): boolean {
  return point.x >= outer.x && point.y >= outer.y
    && point.x < outer.x + outer.width && point.y < outer.y + outer.height;
}

function intersects(left: OilBodyVfxRect, right: OilBodyVfxRect): boolean {
  return left.x < right.x + right.width && right.x < left.x + left.width
    && left.y < right.y + right.height && right.y < left.y + left.height;
}

function contains(rect: OilBodyVfxRect, point: OilBodyVfxPoint): boolean {
  return point.x >= rect.x && point.x < rect.x + rect.width
    && point.y >= rect.y && point.y < rect.y + rect.height;
}

function forEachPoint(rect: OilBodyVfxRect, visit: (point: OilBodyVfxPoint) => void): void {
