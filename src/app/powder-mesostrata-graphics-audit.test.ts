...
describe('powder mesostrata graphics audit', () => {
  it('covers exactly the proposed settled smooth powder family', () => {
    expect(POWDER_MESOSTRATA_GRAPHICS_ATLAS.map(({ material }) => material))
      .toEqual([Material.Sand, Material.Stone, Material.Clay, Material.Concrete]);
    expect(POWDER_MESOSTRATA_GRAPHICS_AUDIT.authoredHoles).toHaveLength(4 * 7 * 7);
    expect(POWDER_MESOSTRATA_GRAPHICS_AUDIT.thinColumns).toHaveLength(4 * 52);
    expect(POWDER_MESOSTRATA_GRAPHICS_AUDIT.unstablePairs).toHaveLength(8);
  });
