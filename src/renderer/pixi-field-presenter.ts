...
void main() {
  ...
  if (material < 0.5) {
    ...
    if (liquid.a > 0.28) {
      ...
      if (uMaterialBodyFinish > 0.5) {
        ...
        MaterialBodyFinishResponse liquidFinishProfile = materialBodyFinishParameters(
      ...
      if (uGasFieldLighting > 0.5) {
        ...
        MaterialBodyFinishResponse gasFinishProfile = materialBodyFinishParameters(
          2.0, gasBodyFinishOptics(0.0, gasIdentityStyle), uMaterialBodyFinish
        );
        vec4 gasFinishResponse = gasFinishProfile.optics;
        gas = applyMaterialBodyFinish(
          gas, 2.0, gasFinishResponse, gasFinishProfile.roughness, atmosphere.a, gasFinishDepth,
  ...
  if (family == 1.0) {
    ...
    if (uGasFieldLighting > 0.5) {
      ...
      color = applyMaterialBodyFinish(
        color, 2.0, gasFinishResponse, gasFinishProfile.roughness, gasDensity, smoothstep(0.035, 0.62, gasDensity),
        gasFinishSlope, 1.0, uMaterialBodyFinish, 0.0
      );
  ...
  if (family == 2.0 && optics != 4.0) {
    ...
    if (connectedBodyLiquid && traits < 0.5 && !materialEmissive
      ...
      vec4 liquidFinishResponse = liquidFinishProfile.optics;
      color = applyMaterialBodyFinish(
        color, 1.0, liquidFinishResponse, liquidFinishProfile.roughness, density, liquidFinishDepth, liquidSlope,
        liquidFinishEligibility, uMaterialBodyFinish, 0.0
      );
      color = applyMaterialVolumeLobe(
...
uniform float uNobleGasCoreReliefVfx;
...
void main() {
  ...
  if (wallOnly > 0.5) {
    ...
    vec4 gasFinishResponse = gasFinishProfile.optics;
    color = applyMaterialBodyFinish(
      color, 2.0, gasFinishResponse, gasFinishProfile.roughness, gasShadeDensity, opticalDepth, volumeSlope,
    ...
    if (uGasBodyVfx > 0.5) {
      ...
      // E42: a dense exact Hydrogen cloud currently converges on E04's generic
      // CleanGas body while its fine style-5 motif deliberately fades under
      // atmosphere ownership. Recompose only that propagated style into a
      // broad blue-white transmission fold with an opposing cool pocket. The
      // existing E04 billow, third carrier, cardinal relief, curvature, and
      // connected-density proof are the complete input set. This changes RGB
      ...
      // Canvas and compact true 8x retain their established Hydrogen identity.
      if (uHydrogenBodyVfx > 0.5 && uGasIdentityStyling > 0.5
        && wall < 0.5 && !materialEmissive) {
        float hydrogenBodyStyle = floor(gasStyleState.r * 255.0 + 0.5);
        float hydrogenBodyOwner = 1.0
          - step(0.5, abs(hydrogenBodyStyle - 5.0));
        if (hydrogenBodyOwner > 0.5) {
          float hydrogenBodyDensity = smoothstep(
            0.12, 0.42, atmosphereState.a
          ) * (1.0 - smoothstep(0.82, 1.0, atmosphereState.a) * 0.24);
          float hydrogenBodySupport = hydrogenBodyOwner * gasVfxBodySupport
            * smoothstep(0.16, 0.52, cloudNeighbourMean)
            * hydrogenBodyDensity;
          float hydrogenBodyPhase = clamp(
            gasVfxBillow * 0.36 + gasVfxWaveC * 0.64
              + gasDirectionalRelief * 0.12 + gasCurvature * 0.06,
            -1.0, 1.0
          );
          float hydrogenBodyCrown = hydrogenBodySupport
            * (max(hydrogenBodyPhase, 0.0) + gasCrown * 0.10);
          float hydrogenBodyPocket = hydrogenBodySupport
            * (max(-hydrogenBodyPhase, 0.0) + gasPocket * 0.08);
          vec3 hydrogenBodyKey = mix(
            vec3(0.58, 0.82, 1.00), vec3(0.82, 0.94, 1.00),
            clamp(0.52 + gasDirectionalRelief * 0.26, 0.0, 1.0)
          );
          color += (vec3(1.12) - clamp(color, 0.0, 1.12))
      ...
      if (uCarbonDioxideBodyVfx > 0.5 && uGasIdentityStyling > 0.5
        ...
        if (carbonDioxideBodyOwner > 0.5) {
          ...
          // E73: the accepted E44 body can still converge on one grey lozenge
          // at fit view. Cross its already-live billow and third wave into a
          // second broad fold, then admit that response only in the optically
          // deep connected core. This produces an opposing cool transmission
          // shoulder and blue-grey absorptive pocket while preserving E44's
          // soft-rim support. The two scalars are consumed locally; this is
          // RGB-only arithmetic with no sample, wave, resource, clock, alpha, support,
          // silhouette, ownership, topology, or physics decision.
          if (uCarbonDioxideCoreFoldVfx > 0.5
            && uCarbonDioxideBodyVfx > 0.5) {
            float carbonDioxideCoreFold = clamp(
              gasVfxBillow * 1.10 - gasVfxWaveC * 0.46,
              -1.0, 1.0
            );
            float carbonDioxideCoreSupport = carbonDioxideBodySupport
              * smoothstep(0.36, 0.78, opticalDepth)
              * smoothstep(0.72, 0.96, cloudNeighbourMean)
              * (1.0 - smoothstep(0.04, 0.34, abs(gasCurvature)));
            float carbonDioxideCoreCrown = carbonDioxideCoreSupport
              * smoothstep(0.08, 0.72, max(carbonDioxideCoreFold, 0.0));
            float carbonDioxideCorePocket = carbonDioxideCoreSupport
              * smoothstep(0.08, 0.72, max(-carbonDioxideCoreFold, 0.0));
            vec3 carbonDioxideCoreKey = mix(
              vec3(0.52, 0.67, 0.80), vec3(0.64, 0.75, 0.86),
              clamp(0.52 + gasDirectionalRelief * 0.20, 0.0, 1.0)
            );
            color += (vec3(1.10) - clamp(color, 0.0, 1.10))
              * carbonDioxideCoreKey * carbonDioxideCoreCrown * 0.220;
            color *= vec3(1.0) - vec3(0.12, 0.16, 0.22)
              * carbonDioxideCorePocket * 1.200;
          }
        }
      }
      ...
      // Propagated style 1 is the sole owner; sparse carriers, foreign-style
      // gas, walls, and emissive matter are exact no-ops. Semantic authored
      // gaps retain exact topology, though their already-supported atmosphere
      // edge may keep this bounded RGB response. The Smoke side of an unlike-
      // gas seam may do likewise; its foreign side remains raw-RGB exact (the
      // later HDR filter may leave a named one-byte composed footprint).
      // This is bounded RGB arithmetic only: no sample, new wave, clock, field,
      // pass, target, allocation, alpha, support, silhouette, or topology edit.
      if (uSmokeSoftnessVfx > 0.5 && uGasIdentityStyling > 0.5
        && wall < 0.5 && !materialEmissive) {
        float smokeSoftnessStyle = floor(gasStyleState.r * 255.0 + 0.5);
        float smokeSoftnessOwner = 1.0
          - step(0.5, abs(smokeSoftnessStyle - 1.0));
        float smokeSoftnessBody = smokeSoftnessOwner * gasVfxBodySupport
          * smoothstep(0.14, 0.50, cloudNeighbourMean)
          * smoothstep(0.08, 0.34, atmosphereState.a);
        // E04's third carrier spans roughly one full fold across the canonical
        // Smoke body. Reusing it explicitly balances the otherwise negative-
        // biased composite there without inventing another frequency.
        float smokeSoftnessMacroFold = mix(gasVfxBillow, gasVfxWaveC, 0.55);
        float smokeSoftnessLobe = clamp(
          smokeSoftnessMacroFold * 0.76 + gasDirectionalRelief * 0.16
            + gasCurvature * 0.08,
          -1.0, 1.0
        );
        float smokeSoftnessCrown = smokeSoftnessBody
          * (max(smokeSoftnessLobe, 0.0) * 0.125 + gasVfxCrown * 0.040);
        float smokeSoftnessPocket = smokeSoftnessBody
          * (max(-smokeSoftnessLobe, 0.0) * 0.100 + gasPocket * 0.030);
        vec3 smokeSoftnessKey = mix(
          vec3(0.78, 0.66, 0.55), vividColor(gasBase, 1.03), 0.28
        );
        color += (vec3(1.10) - clamp(color, 0.0, 1.10))
      ...
      if (uNobleGasPrismVfx > 0.5 && uNobleGasBillowVfx > 0.5
        ...
        if (noblePrismOwner > 0.5) {
          ...
          // E77 separates convex and recessed structure inside E31's dense
          // exact-Noble core. The existing atmosphere depth, curvature, and
          // cardinal directional relief are its complete geometry: it neither
          // repeats the prismatic macro phase nor invents another noise layer.
          // RGB only; no sample, texture, field, pass, target, upload,
          // allocation, clock, alpha, support, silhouette, ownership,
          // topology, state, or physics decision is added. Canvas and compact
          // true 8x retain E31's established presentation.
          if (uNobleGasCoreReliefVfx > 0.5) {
            float nobleCoreDensity = smoothstep(0.72, 0.98, atmosphereState.a);
            float nobleCoreSupport = noblePrismSupport * nobleCoreDensity
              * smoothstep(0.72, 0.96, cloudNeighbourMean);
            float nobleCoreRelief = clamp(
              gasDirectionalRelief * 1.40
                + (gasCrown - gasPocket) * 0.18,
              -1.0, 1.0
            );
            // Expand the accepted E31 composite around its live Noble albedo
            // instead of sampling or rebuilding the prism phase. Optical depth
            // admits only the connected core; the signed RGB result already
            // contains all established body lighting and therefore gains
            // contrast without a new spatial carrier or flat species tint.
            float nobleCoreLumaDelta = clamp(dot(
              color - gasBase, vec3(0.2126, 0.7152, 0.0722)
            ), -0.12, 0.12);
            float nobleCoreContrast = nobleCoreSupport
              * smoothstep(0.72, 1.0, opticalDepth);
            float nobleCoreContrastGain = mix(
              0.85, 2.20, step(0.0, nobleCoreLumaDelta)
            );
            color *= 1.0 + nobleCoreLumaDelta * nobleCoreContrast
              * nobleCoreContrastGain;
            float nobleCoreCrown = nobleCoreSupport * max(nobleCoreRelief, 0.0)
              * (1.0 - opticalDepth * 0.16);
            float nobleCorePocket = nobleCoreSupport
              * max(-nobleCoreRelief, 0.0)
              * (0.62 + opticalDepth * 0.38);
            color += (vec3(1.14) - clamp(color, 0.0, 1.14))
              * noblePrismSpectrum * nobleCoreCrown * 0.750;
            color *= vec3(1.0)
              - vec3(0.18, 0.42, 0.14) * nobleCorePocket * 0.550;
          }
        }
      }
    ...
    if (family == 4.0) {
      ...
      if (uPowderStyle > 1.5 && surfaceOnly < 0.5 && traits < 0.5 && !materialEmissive) {
        ...
        if (uPowderBodyVfx > 0.5 && halo < 0.5 && wall < 0.5
          ...
          // E52: exact BGLA reuses only E05's already-live broad facet, depth,
          // and contour-retention scalars. Keeping this compact at the proven
          // body site avoids extending the normal shader's live register set,
          // while suppressing cell pepper and giving the shard pack a cool
          // opposing crown/pocket. The later native splinter motif stays sharp.
          if (uBglaBodyVfx > 0.5 && optics == 13.0 && material == 44.0
            && foreignMatterContact < 0.5 && unlikeMaterialContact < 0.5) {
            powderContourTextureRetention = mix(
              powderContourTextureRetention, 0.28, powderBodyGate
            );
            color += (vec3(1.14) - clamp(color, 0.0, 1.14))
              * vec3(0.56, 0.78, 1.00) * powderBodyGate
              * max(powderVfxFacetBalance, 0.0) * 0.20;
            color *= vec3(1.0) - vec3(0.080, 0.063, 0.038)
              * powderBodyGate * (
                max(-powderVfxFacetBalance, 0.0) + powderBodyVolumeDepth * 0.10
              );
      ...
      if (botanicalBodyReplacement > 0.5) {
        ...
        if (material == 10.0) {
          ...
          color += vec3(-0.052, 0.022, -0.060) * leafPigmentBody
            * botanicalPigment;
          // E32: exact PLNT retains a cohesive canopy, but its accepted E28
          // body is too uniform at fit view. One static 2.8-cell octave adds a
          // subordinate lamina grain while the established signed lobe and
          // segmented vein keep that fine carrier organized as leaf tissue.
          // The octave is evaluated only for eligible zero-payload PLNT in a
          // raw-zero or ordinary owner-present state. This normal-shader path
          // remains RGB-only and adds no texture fetch,
          // field, pass, target, allocation, clock, alpha, support, lifecycle
          // state, topology, ownership, or physics decision. Canvas and compact
          // true 8x retain the accepted E28 presentation.
          float plantLamina = material == 10.0
              && botanicalBodyReplacement > 0.5
          ...
          float leafLaminaPhase = leafLaminaSegment
            * clamp(leafPigment / 0.19, -1.0, 1.0);
          float leafLaminaFine = 0.5;
          if (plantLamina > 0.001) {
            leafLaminaFine = botanicalBodyNoise(
              fieldPosition / 2.8 + vec2(13.7, -8.9)
            );
          }
          float leafLaminaCarrier = clamp(
            (leafLaminaFine - 0.5) * 2.0 + leafLaminaPhase * 0.12,
            -1.0, 1.0
          );
          float leafLaminaCrown = max(leafLaminaCarrier, 0.0);
          float leafLaminaPocket = max(-leafLaminaCarrier, 0.0);
              ...
              if (uPlantCanopyInterlockVfx > 0.5) {
                ...
                if (uPlantCanopyHierarchyVfx > 0.5) {
                  ...
                  color += vec3(-0.028, 0.043, -0.036)
                    ...
                    * leafCanopyHierarchyWeight;
                  // E71: the fitted production canopy already owns ample E32/
                  // E53 fine mottling, but still reads as one textured cushion.
                  // Consolidate that accepted detail into overlapping leaf
                  // masses by blending toward E20's already-live leaf albedo,
                  // then relight only with E55/E58's broad fold, rank, overlap,
                  // and environment evidence. This strict E58 child adds no
                  // noise call, sample, texture, field, pass, target, upload,
                  // allocation, clock, alpha, support, silhouette, lifecycle,
                  // topology, ownership, state, or physics decision. Canvas
                  // and compact true 8x retain the accepted E58 presentation.
                  if (uPlantCanopyFoliageVfx > 0.5) {
                    float leafCanopyFoliageFold = clamp(
                      leafCanopyHierarchyFold * 0.68
                        + leafCanopyInterlockFold * 0.20
                        + leafCanopyRank * 0.12,
                      -1.0, 1.0
                    );
                    float leafCanopyFoliageCrown = smoothstep(
                      0.035, 0.66, max(leafCanopyFoliageFold, 0.0)
                    );
                    float leafCanopyFoliagePocket = smoothstep(
                      0.035, 0.66, max(-leafCanopyFoliageFold, 0.0)
                    );
                    float leafCanopyFoliageWeight = plantCanopyMass
                      * (0.72 + leafCanopyOverlap * 0.28);
                    float leafCanopyFoliageBlend = min(
                      0.38,
                      (0.19 + abs(leafCanopyFoliageFold) * 0.16
                        + leafCanopyOverlap * 0.045)
                        * leafCanopyFoliageWeight
                    );
                    vec3 leafCanopyFoliageAlbedo = clamp(
                      leafKey * (
                        0.91 + leafCanopyFoliageCrown * 0.18
                          - leafCanopyFoliagePocket * 0.12
                      )
                        + solidEnvironment
                          * (0.026 + leafCanopyFoliageCrown * 0.050)
                        + vec3(-0.030, 0.044, -0.050)
                          * leafCanopyFoliageFold,
                      0.0, 1.0
                    );
                    color = mix(
...
export class PixiFieldPresenter {
  ...
  private constructor(
    ...
    // E52 is exact-BGLA arithmetic inside E05's settled Smooth proof. Compact
    ...
    // E73 is arithmetic inside E44's exact dense-core proof. Canvas and compact
    // true 8x retain E44 and declare neither this selector nor its branch.
    const carbonDioxideCoreFoldVfxEnabled = outputScale < 8
      && resolveCarbonDioxideCoreFoldVfxEnabled(renderLook);
    ...
    // E77 reuses E31's exact-Noble dense-body proof and field-derived relief.
    // Canvas and compact true 8x declare no selector or parallel branch.
    const nobleGasCoreReliefVfxEnabled = outputScale < 8
      && resolveNobleGasCoreReliefVfxEnabled(renderLook);
    ...
    // E71 consolidates only E58's proven hierarchy into broad exact-PLNT
    ...
    this.uniforms = new UniformGroup({
      ...
      uCarbonDioxideCoreFoldVfx: {
        value: carbonDioxideCoreFoldVfxEnabled ? 1 : 0, type: 'f32',
      },
      ...
      uNobleGasCoreReliefVfx: {
        value: nobleGasCoreReliefVfxEnabled ? 1 : 0, type: 'f32',
      },
    ...
    if (!this.hdrVfxPipeline) {
      ...
      this.uniforms.uniforms.uNobleGasCoreReliefVfx = 0;
  ...
  static async create(
      ...
      await app.init({
        ...
        preserveDrawingBuffer: typeof location !== 'undefined'
          ...
          && (new URLSearchParams(location.search).get('blankAudit') === '1'
            ...
            || new URLSearchParams(location.search).get('nobleGasCoreReliefVfxAudit') === '1'
    ...
    presenter.app.canvas.dataset.carbonDioxideCoreFoldVfx = Number(
      presenter.uniforms.uniforms.uCarbonDioxideCoreFoldVfx
    ) > 0.5 ? 'active' : 'inactive';
    ...
    presenter.app.canvas.dataset.nobleGasCoreReliefVfx = Number(
      presenter.uniforms.uniforms.uNobleGasCoreReliefVfx
    ) > 0.5 ? 'active' : 'inactive';
  ...
  */
  ...
  /**
   * Audit-only causal toggle for E71. It cannot revive E58 and is absent from
   * the compact true-8x shader; normal product state comes from render-look.
   */
  setPlantCanopyFoliageVfxEnabled(enabled: boolean): void {
    if (this.outputScale >= 8
      || Number(this.uniforms.uniforms.uPlantCanopyHierarchyVfx) <= 0.5) {
      this.uniforms.uniforms.uPlantCanopyFoliageVfx = 0;
      this.app.canvas.dataset.plantCanopyFoliageVfx = 'inactive';
      return;
    }
    this.uniforms.uniforms.uPlantCanopyFoliageVfx = enabled ? 1 : 0;
    this.app.canvas.dataset.plantCanopyFoliageVfx = enabled ? 'active' : 'inactive';
    this.renderApplication();
  }
  ...
  private renderPresentationFrame(): void {
    ...
    if (pipeline) {
      try {
        ...
        this.app.canvas.dataset.nobleGasCoreReliefVfx = 'inactive';
