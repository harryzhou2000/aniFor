...
describe('Pixi presenter startup configuration', () => {
  ...
  it('keeps Smooth powder body depth calm, gated, bounded, and topology-neutral', () => {
    ...
    expect(source).toContain('mix(1.0, 1.62, settledMineralRetention)');
    ...
    expect(source).toContain('color += base * grain * vec3(0.154, 0.036, -0.096)');
  ...
  it('keeps the WebGL-only aqueous surface shoulder independently switchable', () => {
    ...
    expect(normal).toContain('vec3(0.022, 0.008, 0.000) * aqueousCoreVolume');
    expect(normal).toContain('float aqueousCoreGlaze = aqueousCoreVolume');
