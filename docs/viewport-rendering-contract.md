...
4. **Backing space:** Canvas/WebGL pixels. The default output scale is 2× per axis, producing 1224×768 backing pixels for 612×384 logical cells. The in-app Detail control and `?renderScale=1|2|4|8` select real per-axis sampling; canonical 8× is 4896×3072. Backing scale must not enter world, brush, pan, or CSS layout math. Probe `MAX_RENDERBUFFER_SIZE`, both `MAX_VIEWPORT_DIMS` axes, and `MAX_TEXTURE_SIZE` before Pixi allocation, explicitly release the probe context, and cap the effective scale against both those device limits and the project budget. Compare width and height independently. The direct-mesh 8× target needs renderbuffer/viewport capacity but no redundant full-size texture; filter-based 1×–4× must also fit the texture limit. WebGL 8× disables redundant MSAA and extra high-quality probes, draws the semantic shader directly on one mesh, uses only a 2× startup compatibility Canvas, and permits only one fenced GPU frame in flight while later mutations coalesce. Every temporary Canvas/contour backing is explicitly released after WebGL promotion.
...
- At one fixed desktop CSS viewport, `renderScale=1`, `renderScale=2`, and WebGL `renderScale=8` produce identical canvas rectangles; only the backing changes between 612×384, 1224×768, and 4896×3072. Requested/effective 8× must both be reported as 8 on the supported canonical audit path; a synthetic 4096-wide capability must choose effective 4×, asymmetric 8192×4096 must retain 8×, and a filter scale that exceeds `MAX_TEXTURE_SIZE` must step down before presenter creation.
...
For routine focused checks, use `npm run audit:viewport:desktop`,
`audit:viewport:mobile`, `audit:viewport:live-scale`, or
`audit:viewport:matrix`. They map directly to the narrow browser gates; the
full `audit:browser-input` remains the release-level combined proof.

The dedicated 8× gate also owns and records eight explicit completed presentations before its composed material captures. Audit-owned frames are important for paused/static fixtures: a timer request must not wait for an unrelated animation or dynamic-field refresh. The 8× SwiftShader path uses a `SYNC_GPU_COMMANDS_COMPLETE` audit fence because giant elapsed-GPU queries can remain unavailable indefinitely even after a successful framebuffer; its sequence advances only after the fence signals, and every subsequent 8× capture requests and awaits another completed frame. The fence is discarded after the same bounded 30-second deadline as production startup (10 seconds at lower scales). The gate independently forces that exact stalled-fence branch and requests `WEBGL_lose_context`, requiring both paths to rebuild a 1224×768 Canvas while preserving the semantic marker, non-default camera, CSS geometry, and painted footprint. Timer-query-capable 1×–4× paths retain elapsed-GPU timing. `node scripts/verify-browser-input.mjs --visual-scale-matrix-only` additionally compares WebGL 1×/2×/4×/8× and Canvas 1×/2×/4× at one CSS viewport, requiring exact backing sizes, identical canvas rectangles, and bounded solid, translucent, and liquid support/depth/hue. The current SwiftShader reference completed canonical 8× frames at about `2940/2968/3005 ms` median/p90/max; the earlier sub-millisecond CPU-submission number was not GPU completion evidence.
