...
Timing output is diagnostic rather than a cross-machine pass/fail threshold. It includes Canvas surface-lighting and deliberately pessimistic full-612×384 energy-core and trait-core passes as distinct stages; both reuse fixed scratch storage and do not contribute to `combinedAllocatedBytes`. Ordinary scenes run trait styling only for cells whose packed role byte is nonzero. Unit tests enforce the allocation ceilings and scheduling rate; the browser render lab remains the visual and runtime-GLSL gate.
