...
Normal WebGL now gives only proven dense ordinary liquid and opaque solid interiors a final, hue-preserving ambient lift. It is capped at five framebuffer RGB bytes and uses already-computed liquid depth/cardinal support or solid interior/optical depth; boundaries, droplets, seams, granular matter, thin structure, traits, emission, gas, energy, walls, and transparent matter remain unchanged. The default-on `setDenseBodyAmbientFill` audit control proves deep Water and Metal brighten on an exact off→on→off cycle at DPR 1 while protected controls retain exact RGB/support/alpha. `npm run audit:dense-body-ambient:8x` proves the compact true-8× mesh deliberately remains byte-identical and resource-neutral.
