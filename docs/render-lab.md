...
Liquid surface lighting follows shared-field support even though the existing semantic/reconstruction path still owns alpha. Canvas attenuates binary-neighbour contour noise as centre field alpha becomes cohesive and permits local surface glints only under empty semantic air with low field support above; unlike species remain hard RGB seams. Lava's continuous body glow stays in the broad emission field while its twice-composited local accent is restricted to that field-owned top surface. WebGL reuses the already sampled centre and four cardinal liquid alpha values to suppress semantic micro-normals only inside connected pools. Sparse droplets keep low cardinal support, and neither backend changes silhouette or reads neighbouring liquid RGB.
