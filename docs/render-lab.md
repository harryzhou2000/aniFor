...
- Lower right: a five-column profile matrix. Its rows cover rigid surfaces; organic/growing matter; radioactive solids; then energy, devices, and force/special fields. Deliberate sparse holes expose each family's reconstruction behavior without relying on phase overrides. Equal-height warm and cool source strips flank the matrix, while its centre column remains a lower-light comparison, so contour response and interior wash are visible in one frame.
...
Timing output is diagnostic rather than a cross-machine pass/fail threshold. It includes the Canvas surface-lighting stress pass as a distinct stage; that pass reuses existing buffers and does not contribute to `combinedAllocatedBytes`. Unit tests enforce the allocation ceilings and scheduling rate; the browser render lab remains the visual and runtime-GLSL gate.
