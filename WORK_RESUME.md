...
- WebGL solid and liquid boundaries now use monotone cubic Hermite weights and analytic derivatives over the existing four compatible-occupancy samples. The kernel has the same 0.5 integral as linear interpolation, adds no texture read/field/buffer/pass, and is covered by exhaustive binary-corner symmetry/area tests. Native walls, gas, energy, semantic occupancy, and unlike-liquid RGB selection stay outside this contour estimator.
- The shader now derives stronger key/fill diffuse light, broad specular response, Fresnel-like edge reflection, bounded environment tint, liquid depth transmission, and caustic variation from its existing slopes/support. A thirteenth `TranslucentRigid` optics class gives Glass, Ice, Quartz/QRTZ, DRIC, NICE, and RIME reduced opacity with cool transmission/reflection response in both WebGL and Canvas without changing physics or material selection.
...
- The render lab now has an exact rounded Metal body and exact split Water/Oil capsule. The browser audit blank-differences both silhouettes, asserts one dominant component and bounded compactness, directly samples the Water/Oil seam, and explicitly gates Glass/Ice visibility, cool tint, macro relief, and clipping. Compactness remains a secondary fragmentation check; fixed-world-grid area/displacement and stair-energy gates are still the stronger queued proof.
...
- TypeScript and all 44 test files / 225 tests pass with the new coverage, contact, and optics classes. The 765-module production build and 20-file static closure pass, as do fresh WebGL and forced-Canvas audits over the same exact fixture revision. The tranche is ready for checkpoint commit/review; push, manual cached deployment, and live Pages validation still remain.
