...
- The exact shallow render-lab slope still exposes terraces when its height changes only once across many columns. Supersampling and the one-cell edge make each transition cleaner but do not infer the long-range tangent. The next visual tranche should build a bounded slope-aware height/SDF or contour representation with an explicit stair-energy gate, rather than increasing blur or claiming that supersampling alone solves geometry.

## Next sequence

1. Replace the remaining shallow-heap terraces with a bounded slope-aware powder surface reconstruction (height/SDF or equivalent), preserving temporal ownership, one-cell maximum presentation growth, mixed-material exclusivity, loose round grains, and Canvas/WebGL parity; add deep-zoom stair-energy evidence.
